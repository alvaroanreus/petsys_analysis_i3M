...
def energy_sel_cut(reader, infiles, perc_cut, flag_ref=False):
    max_slab = select_max_energy(ChannelType.TIME)
    energy_mm_dict = {}
    ldat_mm_dict = {
        "_14.ldat": [12, 13, 14, 15],
        "_15.ldat": [12, 13, 14, 15],
        "_16.ldat": [12, 13, 14, 15],
        "_44.ldat": [8, 9, 10, 11],
        "_45.ldat": [8, 9, 10, 11],
        "_46.ldat": [8, 9, 10, 11],
        "_67.ldat": [4, 5, 6, 7],
    ...
    for key in energy_mm_dict.keys():
        ...
        lim_inf = (1 - perc_cut) * mu
        lim_sup = (1 + perc_cut) * mu
...
def position_histograms(
    ...
    def _plotter(evt: tuple[list, list]) -> None:
        ...
        for sm_info in evt:
            ...
            if erange(eng):
                ...
                if (
                    Y < ybins[-1]
                    and (bn_idx := np.searchsorted(ybins, Y, side="right") - 1) >= 0
                ):
                    _plotter.yspecs[tuple(sm_mm_slab_idx)][bn_idx] += 1
                    if (
                        DOI < dbins[-1]
                        and (bn_idx := np.searchsorted(dbins, DOI, side="right") - 1)
                        >= 0
                    ):
    ...
    _plotter.yspecs = {
        (sm, mm, slab_idx): np.zeros(ybins.shape[0] - 1, np.uint)
        for slab_idx in range(slab_idx_nums)
        for mm in mm_nums
        for sm in range(sm_nums)
    }
    _plotter.dspecs = {
        (sm, mm, slab_idx): np.zeros(dbins.shape[0] - 1, np.uint)
        for slab_idx in range(slab_idx_nums)
        for mm in mm_nums
        for sm in range(sm_nums)
    }
    ...
    _plotter.slab_id = {
        (SM, MM, SLAB): 0
        for SLAB in range(slab_idx_nums)
        for MM in mm_nums
        for SM in range(sm_nums)
    }
    return _plotter
...
if __name__ == "__main__":
    ...
    perc_cut = float(conf.get("output", "perc_cut", fallback="0.2"))

    ybins = np.arange(
        *map(float, conf.get("output", "ybinning", fallback="-13,13,0.2").split(","))
    )
    dbins = np.arange(
        *map(float, conf.get("output", "doibinning", fallback="0,21,0.2").split(","))
    )
    ...
    energy_cut_dict = energy_sel_cut(reader, infiles, perc_cut, flag_ref)
    # print(energy_cut_dict)

    plotter = position_histograms(
        batch_size, ybins, dbins, pos_rec, 8, energy_cut_dict, chan_map
    )
    for fn in infiles:
        print(f"Reading {fn}")
    ...
    if plotter.sl_count != 0:
        ...
        for i in range(plotter.sl_count):
            if (
                xy_pos[i][1] < ybins[-1]
                and (bn_idx := np.searchsorted(ybins, xy_pos[i][1], side="right") - 1)
                >= 0
            ):
                plotter.yspecs[tuple(plotter.sm_mm_slab_idx[i])][bn_idx] += 1
                if (
                    DOI[i] < dbins[-1]
                    and (bn_idx := np.searchsorted(dbins, DOI[i], side="right") - 1)
                    >= 0
                ):
                    plotter.dspecs[tuple(plotter.sm_mm_slab_idx[i])][bn_idx] += 1

    ydf = pd.DataFrame(
        shift_to_centres(ybins).round(2), columns=["bin_centres_slab_asic"]
    )
    for (sm, mm, slab_idx), hist in plotter.yspecs.items():
        ydf[
            f"{sm}_{mm}_{slab_idx}_{plotter.slab_id[(sm, mm, slab_idx)]}"
        ] = hist  # .sum(axis=1)
        # print(hist)
    ...
    ydf.drop("index", axis=1).sort_values(["SM", "MM", "SLAB", "ASIC"]).to_csv(
    ...
    ddf = pd.DataFrame(
        shift_to_centres(dbins).round(2), columns=["bin_centres_slab_asic"]
    )
    ...
    ddf.drop("index", axis=1).sort_values(["SM", "MM", "SLAB", "ASIC"]).to_csv(
        out_file, sep="\t", index=False
    )
