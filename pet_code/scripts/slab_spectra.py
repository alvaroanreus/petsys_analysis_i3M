...
if __name__ == "__main__":
    ...
    if "Impacts" in filt_type:
        ...
        min_chan = conf.getint("filter", "min_channels")
        singles = "coinc" not in infile
        evt_select = filter_impacts_one_minimod(
            min_chan, chan_map.get_minimodule, singles=singles
        )
        valid_ch = accept_channel(set(chan_map.mapping.index))
        ...
        chan_set = set(
            np.concatenate(
                [
                    chan_map.get_minimodule_channels(*v)
                    for v in np.vstack(np.stack(np.meshgrid(valid_sms, valid_mms)).T)
                ]
            )
        )
        valid_ch = accept_channel(chan_set)
        evt_select = filter_impacts_module_list(
            chan_map.get_minimodule_channels,
            valid_sms,
            valid_mms,
            min_chan,
            singles=singles,
    ...
    bin_edges = np.arange(
        *map(float, conf.get("output", "binning", fallback="9,25,0.2").split(","))
    )
    out_base = os.path.join(
        out_dir, conf.get("output", "out_file", fallback=infile.split(os.sep)[-1])
    )
    ...
    for id, hist in slab_plots.tdist.items():
        ...
        try:
            ...
            plt.plot(
                bcent,
                gvals,
                label=f"Gaussian fit: mu = {round(pars[1], 2)}, sigma = {round(pars[2], 2)}",
            )
        except RuntimeError:
            max_bin = np.argmax(hist)
            minE, maxE = bin_edges[max_bin - 8], bin_edges[max_bin + 8]
            plt.axvspan(
                minE, maxE, facecolor="#00FF00", alpha=0.3, label="Max +- 8 bins"
            )
