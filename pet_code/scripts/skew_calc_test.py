...
def deltat_df():
    ...
    dist_sigma = 10
    dt_dict = {
        "ref_ch": np.zeros(nsamples, dtype=int),
...
def test_peak_position(deltat_df):
    ...
    bias_finder = peak_position(
        hist_bins, 100, pd.Series(np.zeros_like(all_ids), index=all_ids)
    )
...
def test_process_raw_data(TEST_DATA_DIR, TMP_OUT):
    test_file = os.path.join(TEST_DATA_DIR, "20221121_SourceSM1pos8_1000evt_coinc.ldat")
...
def test_calculate_skews_nobias(TEST_DATA_DIR, TMP_OUT):
    test_file = os.path.join(
        TEST_DATA_DIR, "20221121_SourceSM1pos8_1000evt_coinc.feather"
    )
    test_conf = os.path.join(TMP_OUT, "skew.conf")
