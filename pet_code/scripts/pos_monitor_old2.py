        ...
        for sm_info in evt:
            ...
            if erange(eng):
                ...
                if (
                    ...
                    _plotter.yspecs[tuple(sm_mm)][bn_idx] += 1
                    if (
                        DOI < dbins[-1]
                        and (bn_idx := np.searchsorted(dbins, DOI, side="right") - 1)
                        >= 0
                    ):
                        _plotter.dspecs[tuple(sm_mm)][bn_idx] += 1
                _plotter.sl_count = 0
                _plotter.sm_mm.fill(0)
            # print("--------------")

    _plotter.yspecs = {
        (sm, mm): np.zeros(ybins.shape[0] - 1, np.uint)
        for mm in mm_nums
        for sm in sm_nums
    }
    _plotter.dspecs = {
        (sm, mm): np.zeros(dbins.shape[0] - 1, np.uint)
        for mm in mm_nums
        for sm in sm_nums
    }
...
if __name__ == "__main__":
    ...
    emin_max = tuple(
        map(float, conf.get("output", "elimits", fallback="42,78").split(","))
    )
    ybins = np.arange(
    ...
    plotter = position_histograms(
        batch_size, ybins, dbins, pos_rec, 8, emin_max, chan_map
    )
    for fn in infiles:
        print(f"Reading {fn}")
    ...
    ydf.drop("index", axis=1).sort_values(["SM", "MM"]).to_csv(
    ...
    ddf.drop("index", axis=1).sort_values(["SM", "MM"]).to_csv(
        out_file, sep="\t", index=False
    )
