...
def output_time_plots(
    histos: ChannelEHistograms,
    cal_name: str,
    out_dir: str,
    file_name: str,
    min_stats: int,
    ...
    for id, dist in histos.tdist.items():
        slab_sum += dist
        try:
            *_, fit_pars, _, _ = fit_gaussian(
                dist, histos.edges[htype], min_peak=min_stats, pk_finder="peak"
            )
    ...
    plt.savefig(
        os.path.join(
            out_dir,
            file_name.split(os.sep)[-1].replace(".ldat", f"{cal_name}_timeEngMu.png"),
        )
    )
    plt.clf()
    ...
    plt.savefig(
        os.path.join(
            out_dir,
            file_name.split(os.sep)[-1].replace(".ldat", f"{cal_name}_timeEngSig.png"),
        )
    )
    plt.clf()

    bcent, gvals, pars, _, _ = fit_gaussian(
        slab_sum, histos.edges[htype], pk_finder="peak"
    )
    plt.errorbar(bcent, slab_sum, yerr=np.sqrt(slab_sum), label="Energy distribution")
    plt.plot(
        bcent,
        gvals,
        label=f"Fit: mu = {round(pars[1], 3)}, sigma = {round(pars[2], 3)}",
    )
    ...
    plt.savefig(
        os.path.join(
            out_dir,
            file_name.split(os.sep)[-1].replace(".ldat", f"{cal_name}_timeAllDist.png"),
        )
    )
    plt.clf()
    ##
...
def output_energy_plots(
    ...
    txt_file = os.path.join(
        out_dir,
        file_name.split(os.sep)[-1].replace(".ldat", f"{cal_name}_MMEngPeaks.txt"),
    )
    with open(txt_file, "w") as peak_out:
        ...
        for sm in no_super:
            ...
            for mm in range(fig_cols * fig_rows):
                ...
                try:
                    bcent, gvals, pars, _, _ = fit_gaussian(
                        dist, histos.edges[htype], pk_finder="peak"
                    )
                except RuntimeError:
                    continue
                fig_ax[1].flatten()[mm].errorbar(
                    bcent, dist, yerr=np.sqrt(dist), label="dataset"
                )
                fig_ax[1].flatten()[mm].plot(
                    bcent,
                    gvals,
                    label=f"Fit: mu = {round(pars[1], 3)}, sigma = {round(pars[2], 3)}",
                )
                ...
                peak_out.write(
                    f"{sm}\t{mm}\t{round(pars[1], 3)}\t{round(pars[2], 3)}\n"
                )
            out_name = os.path.join(
                out_dir,
                file_name.split(os.sep)[-1].replace(
                    ".ldat", f"{cal_name}_MMEngs_sm{sm}.png"
                ),
            )
    ...
    plt.savefig(
        os.path.join(
            out_dir,
            file_name.split(os.sep)[-1].replace(".ldat", f"{cal_name}_mmEngMu.png"),
        )
    )
    plt.clf()
    bins = min(sig_vals) - 2, max(sig_vals) + 2, np.diff(histos.edges[htype][:2])[0]
    ...
    plt.savefig(
        os.path.join(
            out_dir,
            file_name.split(os.sep)[-1].replace(".ldat", f"{cal_name}_mmEngSig.png"),
        )
    )
    plt.clf()

    bcent, gvals, pars, _, _ = fit_gaussian(
        all_eng, histos.edges[htype], pk_finder="peak"
    )
    ...
    plt.savefig(
        os.path.join(
            out_dir,
            file_name.split(os.sep)[-1].replace(".ldat", f"{cal_name}_engAllDist.png"),
        )
    )
    plt.clf()
    ##
...
if __name__ == "__main__":
    ...
    if time_cal and eng_cal:
        ...
        cal_name = "_calEng_"
    cal_func = calibrate_energies(
        chan_map.get_chantype_ids, time_cal, eng_cal, eref=eref
    )

    ebins = np.arange(
        *map(float, conf.get("output", "ebinning", fallback="0,300,1.5").split(","))
    )
    tbins = np.arange(
        *map(float, conf.get("output", "tbinning", fallback="9,25,0.2").split(","))
    )
    out_dir = conf.get("output", "out_dir")
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)

    reader = read_petsys_filebyfile(
        chan_map.ch_type, sm_filter=evt_filter, singles=singles
    )
    ...
    for fn in infiles:
        ...
        for evt in map(cal_sel, reader(fn)):
            sm_mm = tuple(
                map(
                    lambda i: (
                        chan_map.get_supermodule(i[0][0]),
    ...
    if args["-j"]:
        fn = os.path.join(
            out_dir, "combinedstats.ldat"
        )  # Review, better to use config variable?
        output_time_plots(plotter, cal_name, out_dir, fn, min_stats)
        output_energy_plots(plotter, cal_name, out_dir, fn, map_file, sm_nums)
