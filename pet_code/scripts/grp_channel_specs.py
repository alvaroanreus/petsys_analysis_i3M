...
def slab_plots(
    ...
    with open(peak_out, "w") as par_out:
        ...
        for id, s_vals in plot_source.tdist.items():
            ...
            try:
                (bcent, g_vals, fit_pars, cov, chi_ndf) = fit_gaussian(
                    diff_data,
                    bin_edges,
                    yerr=bin_errs,
                    min_peak=min_stats,
                ...
                if (
                    fit_pars[1] <= bin_edges[5]
                    or fit_pars[1] >= bin_edges[-5]
                    or chi_ndf > 5
                ):
                    raise RuntimeError
            except RuntimeError:
            ...
            par_out.write(
                f"{id}\t{round(fit_pars[1], 3)}\t{round(mu_err, 3)}\t{round(fit_pars[2], 3)}\t{round(sig_err, 3)}\n"
            )
            if id in monitor_ids:
                plt.errorbar(
                    bcent,
                    diff_data[min_indx:],
                    yerr=bin_errs[min_indx:],
                    label="distribution",
                )
                plt.plot(
                    bcent,
                    g_vals,
                    label=f"Gaussian fit mu = {round(fit_pars[1], 2)}, sigma = {round(fit_pars[2], 2)}",
                )
                plt.legend()
...
def refit_slab(
    ...
    if min_x:
        ...
        try:
            bcent, g_vals, fit_pars, cov, _ = fit_gaussian(
                diff_data, bin_edges[indx:], yerr=bin_errs, min_peak=int(min_peak * 0.7)
            )
        except RuntimeError:
            fail_plot(
                out_file, id, bin_edges, source, nosource, np.sqrt(source + nosource)
            )
            return None
    else:
    ...
    with open(peak_out, "w") as par_out:
        ...
        for id, vals in plot_source.edist.items():
            ...
            if p_indx <= 2 * nbin_fit:
                ...
                continue
            mask = (bin_cent > bin_cent[p_indx] - nbin_fit * bin_wid) & (
                bin_cent < bin_cent[p_indx] + nbin_fit * bin_wid
            )
            try:
                p0 = [hdiff[p_indx], bin_cent[p_indx], 3]
                fit_pars, cov = curve_fit_fn(
                    lorentzian, bin_cent[mask], hdiff[mask], np.sqrt(hdiff[mask]), p0
                )
                av_diff = fit_pars[1]
                av_err = np.sqrt(cov[1, 1])
                if av_err > 1.0:
                    av_diff, av_err = weighted_average(
                        plt, bin_cent, hdiff, hdiff_err, mask
                    )
            except RuntimeError:
                av_diff, av_err = weighted_average(
                    plt, bin_cent, hdiff, hdiff_err, mask
            ...
            if id in monitor_ids:
                ...
                plt.plot(
                    bin_cent, lorentzian(bin_cent, *fit_pars), label="Gaussian fit"
                )
...
def channel_plots(config: configparser.ConfigParser, infiles: list[str]) -> tuple:
    """
    ...
    esum_bins = np.arange(
        *map(
            float, config.get("output", "esum_binning", fallback="0,300,1.5").split(",")
        )
    )
    tbins = np.arange(
        *map(float, config.get("output", "tbinning", fallback="5,30,0.2").split(","))
    )
    ebins = np.arange(
        *map(float, config.get("output", "ebinning", fallback="7,40,0.4").split(","))
    )
...
def average_error(
    x: np.ndarray, y: np.ndarray, yerr: np.ndarray, ysum: np.ndarray
) -> np.ndarray:
    ...
    if ntime:
        ...
        with open(out_base + "timeSlabPeaks.txt", "a") as tout:
            for col in dist_df.columns[dist_df.columns.str.contains("_err")]:
                ...
                try:
                    mu, mu_err, sig, sig_err = refit_channel(
                        bin_centres, dist_df[col[:-4]], dist_df[col], gaussian, 8
                    )
                    tout.write(
                        f"{col[2:-4]}\t{round(mu, 3)}\t{round(mu_err, 3)}\t{round(sig, 3)}\t{round(sig_err, 3)}\n"
                    )
                except RuntimeError:
    ...
    if neng:
        ...
        with open(out_base + "eChannelPeaks.txt", "a") as eout:
            for col in dist_df.columns[dist_df.columns.str.contains("_err")]:
                ...
                try:
                    mu, mu_err, *_ = refit_channel(
                        bin_centres, dist_df[col[:-4]], dist_df[col], lorentzian, 5
                    )
...
def petsys_file(
    map_file: str, tchans: str, echans: str, eref: float, out_name: str
) -> None:
    """
    ...
    cal_fact = pd.concat(
        (
            pd.read_csv(tchans, sep="\t").set_index("ID").MU.map(lambda x: 511.0 / x),
            pd.read_csv(echans, sep="\t").set_index("ID").MU.map(lambda x: eref / x),
        )
    ).sort_index()
    ...
    ch_gain = pd.DataFrame(
        [[1.0, 1.0, 1.0]], columns=out_cols[5:-1], index=all_ids
    ).reset_index()
    ch_gain[out_cols[:4]] = np.row_stack(
        ch_gain.id.map(np.vectorize(get_electronics_nums))
    )
    ch_gain = (
        pd.merge(ch_gain, cal_fact.round(3).reset_index(), on="id", how="left")
        .rename(columns={"MU": out_cols[-1]})
        .fillna(1.0)
    )
...
if __name__ == "__main__":
    ...
    nslab_bad, tname = slab_plots(
        out_file, plotS, plotNS, min_peak, pk_finder=pk_finder, monitor_ids=monitor_id
    )
    neng_bad, ename = energy_plots(
        out_file, plotS, plotNS, min_peak, monitor_ids=monitor_id
    )

    ## Review suspect distributions?
    if nslab_bad > 0 or neng_bad > 0:
        review = input(
            f"There are {nslab_bad} suspect time channels and {neng_bad} energy channels, review now? y/[n]"
        )
        if review:
