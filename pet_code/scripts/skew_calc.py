...
def geom_loc_point(file_name: str, ch_map: ChannelMap, source_yml: dict) -> tuple:
    ...
    """
    (SM_lab, mM_nums, s_pos) = get_references(file_name, source_position(source_yml))
    mm_channels = np.concatenate(
        [ch_map.get_minimodule_channels(SM_lab, mm) for mm in mM_nums]
    )
    valid_ids = set(
        filter(lambda x: ch_map.get_channel_type(x) is ChannelType.TIME, mm_channels)
    )
...
def geom_loc_bar(file_name: str, ch_map: ChannelMap, source_yml: dict) -> tuple:
    ...
    ids = np.concatenate(
        [
            ch_map.get_minimodule_channels(*v)
...
def process_raw_data(
    file_list: list[str], config: configparser.ConfigParser, ch_map: ChannelMap
) -> list[str]:
    """
    ...
    if setup == "pointSource":
        ...
        with open(config.get("mapping", "source_pos")) as s_yml:
            ...
            geom_func = partial(geom_loc_point, ch_map=ch_map, source_yml=yml_positions)
    elif setup == "barSource":
        with open(config.get("mapping", "source_pos")) as s_yml:
            yml_positions = yaml.safe_load(s_yml)
            geom_func = partial(geom_loc_bar, ch_map=ch_map, source_yml=yml_positions)
    else:
    ...
    dt_calc = partial(
        corrected_time_difference, impact_sel=cal_and_select, energy_sel=eselect
    )
    ...
    for fn in file_list:
        ...
        dt_gen = map(
            dt_calc(ref_ch=ref_ch, geom_dt=geom_dt), pet_reader(sm_filter=filt)(fn)
        )
...
def calculate_skews(
    ...
    bias_calc = peak_position(
        np.arange(*hist_bins), min_stats, skew_values, monitor_id, out_name
    )
    for fn in file_list:
...
def peak_position(
    ...
    def calculate_bias(delta_t: pd.DataFrame) -> float:
        ...
        bin_vals, bin_edges = np.histogram(
            delta_t.corr_dt.values + skew_corr, bins=hist_bins
        )
        try:
...
def output_plot(
    ...
    """
    plt.errorbar(
        shift_to_centres(dt_binedges),
        dt_data,
        yerr=np.sqrt(dt_data),
        label="distribution",
    )
...
if __name__ == "__main__":
    ...
    out_skew = (
        input_files[0].split(os.sep)[-1].split("_")[0] + f"_skew_{total_iter}iter.txt"
    )
    ...
    if first_it != 0:
        ...
        skew_vals = pd.Series(0, index=all_ids.sort_values(), name="tOffset (ps)")
        if not args["-r"]:
            chunks = [
                (file_set, conf, ch_map)
