...
if __name__ == "__main__":
    ...
    for id in ch_map.mapping.index:
        if id not in evt_negs.keys():
            print(
                f"Channel {id} has no data. SM{ch_map.get_supermodule(id)}, mM{ch_map.get_minimodule(id)}."
            )
            continue
        prop_negative = np.count_nonzero(evt_negs[id]) / len(evt_negs[id])
        if prop_negative > 0.01:
            print(
                f"More than 1% ({round(prop_negative*100, 3)}%) of energy values are <= 0 for channel {id}. SM{ch_map.get_supermodule(id)}, mM{ch_map.get_minimodule(id)}."
            )
