...
if __name__ == "__main__":
    ...
    print(
        ...
        time_mu,
        ", EThresh = ",
        np.searchsorted(time_EThresh, time_mu),
        ", T1 = ",
        np.searchsorted(time_TThresh, time_mu),
    )
    ...
    with open(output, "w") as thresh_out:
        ...
        for id, typ in sorted(ch_types.items()):
            ...
            if typ is ChannelType.TIME:
                ...
                if peak_pos:
                    vth_t1 = max(
                        0, np.searchsorted(time_TThresh, peak_pos) - 1 + time_TOffset
                    )
                    vth_t2 = max(
                        0, np.searchsorted(time_T2Thresh, peak_pos) - 1 + time_T2Offset
                    )
                    vth_e = max(
                        0, np.searchsorted(time_EThresh, peak_pos) - 1 + time_EOffset
                    )
                ...
                if peak_pos:
                    vth_t2 = max(
                        0, np.searchsorted(eng_TThresh, peak_pos) - 1 + eng_TOffset
                    )
                    vth_e = max(
                        0, np.searchsorted(eng_EThresh, peak_pos) - 1 + eng_EOffset
                    )
            thresh_out.write(
                f"{portID}\t{slaveID}\t{chipID}\t{channelID}\t{vth_t1}\t{vth_t2}\t{vth_e}\n"
            )
