...
if __name__ == "__main__":
    ...
    time_ch, eng_ch, mm_map, *_ = read_ymlmapping(
        "pet_code/test_data/SM_mapping_corrected.yaml"
    )

    evt_filt = filter_minch(4, eng_ch)
    reader = read_petsys_filebyfile(mm_map, sm_filter=evt_filt, singles=True)
    print(
        f"Checking for events with channel {chan_test} in Q range ({q_low}, {q_high})"
    )
    occ_count = 0
    max_chan = 0
    for sm, _ in reader(infile):
        ...
        if chan_found:
            ...
            sums = np.fromiter(
                (sm_arr[(sm_arr[:, 1] == mm) & e_chan, 3].sum() for mm in MMs), float
            )
    ...
    print(
        f"Channel in event {occ_count} times of which {max_chan} it is the first/max channel"
    )
