...
def brain_map(nFEM: int, chan_per_mm: int, tchans: list, echans: list) -> Iterator:
    brain_gen = brain_sm_gen(
        nFEM, chan_per_mm, tchans, echans, {0: [0, 0, 0], 2: [0, 0, 3]}
    )
    for i in (0, 2):
...
def channel_sm_coordinate(
    mm_rowcol: int, ch_indx: int, chtype: ChannelType, geom: TbpetGeom
) -> tuple[float, float]:
    """
...
def brain_sm_gen(
    nFEM: int, chan_per_mm: int, tchans: list, echans: list, sm_to_febport: dict
) -> Callable:
    """
    ...
    def _sm_gen(sm_no: int) -> Iterator:
        ...
        for i, (tch, ech) in enumerate(zip(tchans, echans)):
            ...
            if half == 1:
                ...
                eindx = (
                    31 - i % chan_per_sec if half_sec == 0 else 63 - i % chan_per_sec
                )
                if half_sec == 0:
...
def sm_gen(
    ...
    def _sm_gen(sm_no: int) -> Iterator:
        ...
        for i, (tch, ech) in enumerate(zip(tchans, echans)):
            ...
            mm = i // chan_per_mm
            loc_x, loc_y = channel_sm_coordinate(
                i // 32, i % 32, ChannelType.TIME, geom
            )
            yield id, "TIME", mm, loc_x, loc_y
            id = ech + sm_min_chan  # sm_no * nFEM
            mm = geom.mM_energyMapping[mm]
            loc_x, loc_y = channel_sm_coordinate(
                i // 32, i % 32, ChannelType.ENERGY, geom
            )
            yield id, "ENERGY", mm, loc_x, loc_y
...
def local_translation(
    df: pd.DataFrame, sm_r: float, sm_half_x: float, sm_half_y: float
) -> np.ndarray:
    """
    ...
    """
    coords = np.vstack(
        (np.full(df.shape[0], sm_r), df.local_x - sm_half_x, -df.local_y + sm_half_y)
    )
    return coords.T
...
def single_ring(
    ...
    if isinstance(sm_geom, TbpetGeom):
        ...
        superm_gen = brain_sm_gen(nFEM, chan_per_mm, tchans, echans, sm_feb)
    ...
    def ring_gen() -> Iterator:
        ...
        for sm in range(first_sm, first_sm + sm_geom.sm_per_ring):
            sm_local = pd.DataFrame((ch for ch in superm_gen(sm)), columns=local_cols)
            ...
            sm_local[coords] = sm_local[coords].apply(
                sm_rot.apply, axis=1, result_type="broadcast"
            )
            yield sm_local
...
def n_rings(
    ...
    return pd.concat(
        (ring_at_z(i, rngz) for i, rngz in enumerate(ring_pos)), ignore_index=True
    )
...
if __name__ == "__main__":
    ...
    if geom == "2SM":
        df = pd.DataFrame(
            row_gen(
                nFEM, 8, channel_map["time_channels"], channel_map["energy_channels"]
            ),
            columns=[
                "id",
                "type",
