...
def xyE_binning(
    ...
    sm_specs = {
        sm: {mm: np.zeros(h_shape, np.uint) for mm in range(16)}
...
if __name__ == "__main__":
    ...
    max_sel = (
        select_module(chan_map.get_minimodule)
    ...
    xbins = np.linspace(
        *map(int, conf.get("output", "xbinning", fallback="0,104,500").split(","))
    )
    ybins = np.linspace(
        *map(int, conf.get("output", "ybinning", fallback="0,104,500").split(","))
    )
    ebins = np.linspace(
        *map(int, conf.get("output", "ebinning", fallback="0,300,200").split(","))
    )
    ...
    for fn in infiles:
        ...
        binner, plotter = xyE_binning(
            chan_map, xbins, ybins, ebins, cal_func, max_sel, c_calc
        )
