...
def esum_plot(bin_edges: np.ndarray, kev_conv: Callable):
    def _bin_value(evt):
        for sm in evt:
            eng = kev_conv(sm[0][0]) * sum(
                imp[3] for imp in filter(lambda x: x[1] is ChannelType.ENERGY, sm)
            )
            if (
                eng < bin_edges[-1]
                and (bn_idx := np.searchsorted(bin_edges, eng, side="right") - 1) >= 0
            ):
...
if __name__ == "__main__":
    ...
    mm_eng = convert_to_kev(
        conf.get("calibration", "kev_convert"), chan_map.get_modules
    )

    esum_bins = np.arange(
        *map(float, conf.get("output", "esum_binning", fallback="0,300,1.5").split(","))
    )
