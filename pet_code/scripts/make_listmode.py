...
def equal_and_select(
    chan_map: ChannelMap, time_file: str, eng_file: str, eref: float | None = None
) -> Callable:
    cal_func = calibrate_energies(
        chan_map.get_chantype_ids, time_file, eng_file, eref=eref
    )
...
def cog_loop(
    ...
    def _evt_loop(file_name: str, lm_out: BinaryIO) -> None:
        ...
        for mm_info in map(sel_func, evt_reader(file_name)):
            ...
            if all(eselect(mmE) for mmE in mm_energies) and all(
                max_chans := tuple(map(max_slab, mm_info))
            ):
                sm_nums = (
                    chan_map.get_supermodule(max_chans[0][0]),
                    chan_map.get_supermodule(max_chans[1][0]),
                )
                local_tpos = (
                    chan_map.get_plot_position(max_chans[0][0])[0],
...
def nn_loop(
    ...
    ord_chan = (
        chan_map.mapping[isEng].sort_values(icols).groupby(icols[:-1]).head(nchan).index
    )
    ...
    def _evt_loop(file_name: str, lm_out: BinaryIO):
        ...
        while i != 0:
            ...
            for evt in map(sel_func, islice(read_evt, npred // 2)):
                ...
                for j, sm in enumerate(evt):
                    ...
                    for imp in filter(_is_eng, sm):
                        channel_energies[2 * i + j]["Esignals"][chan_idx[imp[0]]] = imp[
                            3
                        ]

                mm_energies = [
                    kev_conv(chid) * mm["Esignals"].sum()
                    for chid, mm in zip(chids, channel_energies[2 * i : 2 * i + 1])
                ]
                if all(eselect(mmE) for mmE in mm_energies) and all(
                    max_chans := tuple(map(max_slab, evt))
                ):
                    sm_nums = (
                        chan_map.get_supermodule(max_chans[0][0]),
                        chan_map.get_supermodule(max_chans[1][0]),
                    )
                    skew_corr = skew.get(max_chans[1][0], 0) - skew.get(
                        max_chans[0][0], 0
                    )
                    try:
                        ...
                        coincidences[i]["time"] = (
                            max_chans[0][2] - max_chans[1][2] + skew_corr
                        )
                    except KeyError:
                        try:
                            coincidences[i]["pair"] = p_lookup[sm_nums]
                            ...
                            coincidences[i]["time"] = (
                                max_chans[1][2] - max_chans[0][2] - skew_corr
                            )
                            channel_energies[[2 * i, 2 * i + 1]] = channel_energies[
                                [2 * i + 1, 2 * i]
                            ]
                        except KeyError:
                            continue
            ...
            predicted_xy = np.empty(
                npred, np.dtype(("X", np.float32), ("Y", np.float32))
            )
            for i in range(0, npred // 2):  # This way forced by structure.
                pixels = tuple(
                    map(lambda xy: pixel_vals(*xy), predicted_xy[2 * i : 2 * i + 2])
                )
...
if __name__ == "__main__":
    ...
    if "Impacts" in filt_type:
        ...
        evt_select = filter_max_coin_event(
            chan_map.get_supermodule, max_super, min_chan
        )
    else:
    ...
    p_lookup = (
        pd.read_csv(pair_name, sep="\t", names=p_cols)
        .set_index(p_cols[1:])
        .to_dict()["p"]
    )
    ...
    skew = {
        i: 0 for i in chan_map.mapping.index
    }  # pd.Series(0, index=chan_map.mapping.index)#read_skewfile(conf.get('calibration', 'skew')).to_dict()
    ## This last convertor has to be here.
    mm_eng = supermod_energy(
        convert_to_kev(conf.get("calibration", "kev_convert"), chan_map.get_modules)
    )
    ##
    ...
    evt_loop = cog_loop(
        chan_map, evt_reader, sel_func, eselect, pixel_vals, mm_eng, skew, p_lookup
    )
    for fn in infiles:
        # Open output file.
        out_file = os.path.join(
            out_dir, fn.split(os.sep)[-1].replace(".ldat", "_LM.bin")
        )
