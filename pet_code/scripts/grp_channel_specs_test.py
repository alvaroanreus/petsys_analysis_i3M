...
def test_petsys_file(TMP_OUT, TEST_DATA_DIR):
    ...
    np.testing.assert_allclose(
        petsys_df[petsys_df.id.isin(map_type[map_type.type == "TIME"].id)].p3, 511 / 10
    )
    np.testing.assert_allclose(
        petsys_df[petsys_df.id.isin(map_type[map_type.type == "ENERGY"].id)].p3, 1.0
    )
