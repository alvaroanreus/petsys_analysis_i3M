    ...
    if num_sm == 1:
        ...
        for slab_ref in range(num_total_slab_ref):
            ...
            if data_max_f(histo[:, slab_ref]) != 0:
                data_norm[:, slab_ref] = histo[:, slab_ref] / data_max_f(histo[:, slab_ref])
            else:
            ...
            for mm in range(num_mm):
                    for slab in range(num_slab):
                        ...
                        if data_max_f(histo[:, sm, mm, slab]) != 0:
                            data_norm[:, sm, mm, slab] = histo[:, sm, mm, slab] / data_max_f(histo[:, sm, mm, slab])
                        else:
                            cont_0 += 1
...
def category_asign(histo_ref, histo_inf, num_sm, dict_map, x_values, flag_plot = False):
    ...
    for sm in range(num_sm):
        for mm in range(num_mm):
                for slab in range(num_slab):
                    for slab_ref in range(num_total_slab_ref):
                        dif = np.sum(np.abs(histo_inf[:, sm, mm, slab] - histo_ref[:, slab_ref]))
                    ...
                    if flag_plot:
                        ...
                        plt.plot(x_values, histo_inf_plot, label = f'Inf: {label_inf}, {sm},{mm},{slab}')
                        plt.legend()
...
def add_category_asign():
    cat_dict = category_asign(histo_ref_norm, histo_inf_norm, num_sm_inf, dict_map, x_values)
