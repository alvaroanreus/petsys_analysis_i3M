                ...
                if _plotter.sl_count >= batch_size:
                    ...
                    for xy, doi, mod_vals in zip(XY, DOI, _plotter.sm_mm):
                        if (
                            xy[1] < ybins[-1]
                            and (
                                bn_idx := np.searchsorted(ybins, xy[1], side="right")
                                - 1
                            )
                            >= 0
                        ):
                            ...
                            if (
                                doi < dbins[-1]
                                and (
                                    bn_idx := np.searchsorted(dbins, doi, side="right")
                                    - 1
                                )
                                >= 0
                            ):
...
if __name__ == "__main__":
    ...
    if "NN" in rec_type:
        ...
        pos_rec = bunch_predictions(
            system_nm,
            batch_size,
            nn_yfile,
            nn_dfile,
            chan_map.get_minimodule_index,
            chan_map.get_plot_position,
            loc_trans=False,
        )
    else:
