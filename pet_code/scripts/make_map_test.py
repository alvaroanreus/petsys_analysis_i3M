...
def test_channel_sm_coordinate():
    ...
    exp_first = (
        round(geom.mm_edge * 4 - (geom.mm_spacing + geom.slab_width) / 2, 3),
        round(3.5 * geom.mm_edge, 3),
    )
    exp_last = (
        round((geom.mm_spacing + geom.slab_width) / 2, 3),
        round(geom.mm_edge / 2, 3),
    )
...
def test_single_ring_corners(TEST_DATA_DIR, channel_types):
    ...
    exp_corners = (
        pd.concat(
            (
                exp_corners.groupby("Module").head(1),
    ...
    min_max = (
        pd.concat((time_smgrp.head(1), time_smgrp.tail(1)))
...
def test_n_rings(channel_types):
    ...
    exp_cent = {
        0: {"X": 406.4924, "Y": -53.5157, "Z": -305.5312},
