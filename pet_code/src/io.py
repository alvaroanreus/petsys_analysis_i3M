...
def _read_yaml_file(mapping_file: str) -> dict:
    ...
    if (
        ...
        or "time_channels" not in channel_map.keys()
...
def read_ymlmapping(mapping_file: str) -> tuple[set, set, dict, dict, dict]:
    ...
    for sm in range(no_sm):
        for i, (tch, ech) in enumerate(
            zip(channel_map["time_channels"], channel_map["energy_channels"])
        ):
            ...
            centroid_mapping[absolut_ech] = (
                1,
                round(1.6 + 3.2 * (31 - i % 32), 2),
            )  # establish 0 reference at the botom left of the floodmap
            # slab_positions  [absolut_tch] = (slab_x(rc_num, sm), slab_y(row), slab_z(sm))
            slab_positions[absolut_tch] = (
                slab_x(i // 32),
                slab_y(i % 32, sm),
                slab_z(sm),
            )
...
def read_ymlmapping_brain(mapping_file: str) -> tuple[set, set, dict, dict, dict]:
    ...
    for sm in range(no_sm):
        ...
        change_flag = False
        for tch, ech in zip(
            channel_map["time_channels"], channel_map["energy_channels"]
        ):
            absolut_tch = tch + sm * FEM_num_ch
            absolut_ech = ech + sm * FEM_num_ch
            ALLSM_time_ch.add(absolut_tch)
...
def write_event_trace(
    file_buffer: TextIO, sm_map: pd.DataFrame, mm_lookup: Callable
) -> Callable:
    """
    ...
    chan_ord = (
        sm_map[isTIME].sort_values(cols + ["local_x"]).groupby(cols).head(nchan).index,
        sm_map[~isTIME].sort_values(cols + ["local_y"]).groupby(cols).head(nchan).index,
    )
    chan_idx = {
        id: idx % nchan + i * nchan
        for i, chtype in enumerate(chan_ord)
