...
def filter_one_minimod(
    sm1: list[list], sm2: list[list], mm_map: Callable, singles: bool = False
) -> bool:
    """
...
def filter_impacts_one_minimod(
    min_sm: int, mm_map: Callable, singles: bool = False
) -> Callable:
    """
    Event filter based on the minimum energy channel
    hist for each of the two super modules in coincidence
...
def filter_channel_list(valid_channels: np.ndarray) -> Callable:
    ...
    def valid_event(sm1: list[list], sm2: list[list]) -> bool:
        return all(imp[0] in valid_channels for imp in sm1) or all(
            imp[0] in valid_channels for imp in sm2
        )

    return valid_event


def filter_module_list(
    smMm_to_id: Callable, valid_sm: list | np.ndarray, valid_mm: list | np.ndarray
) -> Callable:
    """
    ...
    valid_ids = np.concatenate(
        [smMm_to_id(*v) for v in np.vstack(np.stack(np.meshgrid(valid_sm, valid_mm)).T)]
    )
    return filter_channel_list(valid_ids)


def filter_impacts_channel_list(
    valid_channels: list | np.ndarray,
    min_ch: int,
    mm_map: Callable,
    ...
    def valid_event(sm1: list[list], sm2: list[list]) -> bool:
        return (
            sel_chans(sm1, sm2)
            and filter_one_minimod(sm1, sm2, mm_map)
            and ch_filter(sm1)
            and ch_filter(sm2)
            and sm_filt(sm1, sm2)
        )
...
def filter_impacts_module_list(
    smMm_to_id: Callable,
    sms: list | np.ndarray,
    mms: list | np.ndarray,
    min_sm: int,
...
def filter_event_by_impacts_noneg(min_sm: int, singles: bool = False) -> Callable:
    ...
    def valid_event(sm1: list[list], sm2: list[list]) -> bool:
        return (
            ch1_filter(sm1)
            and ch2_filter(sm2)
            and filter_negatives(sm1)
            and filter_negatives(sm2)
        )
...
def filter_max_coin_event(
    sm_map: Callable, max_sm: int = 2, min_ch: int = 4, singles: bool = False
) -> Callable:
    """
    ...
    def valid_event(sm1: list[list], sm2: list[list]) -> bool:
        return (
            filter_negatives(sm1)
            and filter_negatives(sm2)
            and ch1_filter(sm1)
            and ch2_filter(sm2)
            and max_sm_filter(sm1, sm2)
        )
