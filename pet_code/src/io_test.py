...
def test_read_petsys(TEST_DATA_DIR, module_mapping):
    ...
    infile = os.path.join(
        TEST_DATA_DIR, "petsys_test_TB_SMGathered_120s_4.0OV_10T1_15T2_5E_coinc.ldat"
    )
...
def test_read_petsys_mod1(TEST_DATA_DIR, module_mapping):
    ...
    exp_evt1 = (
        [
            [696, ChannelType.ENERGY, 1137456113288, 5.1196],
    ...
    np.testing.assert_allclose(
        sm0[:, 2:].astype("float"), sm0_exp[:, 2:].astype("float"), rtol=1e-4
    )
    np.testing.assert_allclose(
        sm1[:, 2:].astype("float"), sm1_exp[:, 2:].astype("float"), rtol=1e-4
    )
...
def test_read_ymlmapping(TEST_DATA_DIR):
    ...
    assert all(
        [
            mm3_minX <= xpos < mm3_maxX
            for _, xpos in filter(lambda tpl: tpl[0] == 0, mm3_cent)
        ]
    )
    mm3_minY = 78.4
    mm3_maxY = 103.6
    assert all(
        [
            mm3_minY <= ypos < mm3_maxY
            for _, ypos in filter(lambda tpl: tpl[0] == 1, mm3_cent)
        ]
    )
    ...
    exp_pos = [
        (0, 5, (38.85, -79.45, 0.0)),
        (52, 2, (12.95, -75.95, 0.0)),
        (200, 12, (64.75, -20.95, 0.0)),
        (223, 16, (90.65, -14.55, 0.0)),
        (514, 6, (38.85, -27.65, 123.7971)),
        (567, 2, (12.95, -30.85, 123.7971)),
        (720, 12, (64.75, -85.85, 123.7971)),
        (738, 16, (90.65, -92.25, 123.7971)),
    ]
...
def test_ChannelMap(TEST_DATA_DIR):
    ...
    t_chans = [
        (0, 4, (38.85, -79.45, 0.0)),
        (52, 1, (12.95, -75.95, 0.0)),
        (200, 11, (64.75, -20.95, 0.0)),
        (223, 15, (90.65, -14.55, 0.0)),
        (514, 5, (38.85, -27.65, 123.7971)),
        (567, 1, (12.95, -30.85, 123.7971)),
        (720, 11, (64.75, -85.85, 123.7971)),
        (738, 15, (90.65, -92.25, 123.7971)),
    ]
    ...
    assert all(
        np.allclose(ch_map.get_channel_position(id), pos) for id, _, pos in t_chans
    )

    e_chans = [
        (1, 5, (37.25, -64.75, 0.0)),
        (53, 1, (24.15, -64.75, 0.0)),
        (201, 10, (53.55, -38.85, 0.0)),
        (229, 15, (79.45, -12.95, 0.0)),
        (515, 5, (40.45, -38.85, 123.7971)),
        (572, 5, (27.65, -38.85, 123.7971)),
        (724, 11, (56.75, -90.65, 123.7971)),
        (741, 15, (79.45, -90.65, 123.7971)),
    ]
    ...
    assert all(
        np.allclose(ch_map.get_channel_position(id), pos) for id, _, pos in e_chans
    )
