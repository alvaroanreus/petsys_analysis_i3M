...
def test_mm_energy_spectra_noplots():
    gen_stats = 10000
    mm_e = {
        i: {"energy": np.random.normal(100 + 2 * i, 5, gen_stats)} for i in range(16)
    }
    ...
    accepted_prop = np.fromiter(
        (
            len(v["energy"][exclusions[i](v["energy"])]) / gen_stats
            for i, v in mm_e.items()
        ),
        float,
    )
...
def test_mm_energy_spectra_plots(TMP_OUT):
    ...
    mm_e = {
        i: {
            "energy": np.random.normal(100 + 2 * i, 5, gen_stats),
            "x": np.random.uniform(
                mm_pitch * (i - 1) % 4, mm_pitch * ((i - 1) % 4 + 1), gen_stats
            ),
            "y": np.random.uniform(
                mm_pitch * (3 - (i - 1) // 4), mm_pitch * (4 - (i - 1) // 4), gen_stats
            ),
        }
        for i in range(1, 17)
    }
...
def test_slab_energy_spectra_noplots():
    ...
    gen_stats = 10000
    slab_eng = {
        i: {"energy": np.random.normal(18, 1, gen_stats)} for i in range(nhists)
    }
    ...
    acc_prop = np.fromiter(
        (
            len(v["energy"][excl[k](v["energy"])]) / gen_stats
            for k, v in slab_eng.items()
        ),
        float,
    )
...
def test_group_times(TEST_DATA_DIR, DUMMY_EVT):
    ...
    # Dummy peak filters
    peak_sel = [
        [lambda x: (x > 50) & (x < 60) for _ in range(10)],
...
def test_group_times_slab(DUMMY_EVT):
    ...
    # Dummy peak filters.
    peak_sel = [
        {682: lambda x: (x > 5) & (x < 7), 640: lambda x: (x > 5) & (x < 7)},
        {64: lambda x: (x > 12) & (x < 14), 65: lambda x: (x > 12) & (x < 14)},
    ]
...
def test_ChannelEHistograms_sumhist(DUMMY_EVT):
    ...
    np.testing.assert_array_equal(
        np.histogram(exp_sum, bins=sumbins)[0], e_histos.sum_dist[exp_id]
    )


def test_ChannelEHistograms_maxes(DUMMY_EVT):
    e_histos = ChannelEHistograms(
        np.arange(2, 24, 0.1), np.arange(4, 24, 0.2), np.arange(0, 200, 1.5)
    )
...
def test_sm_floodmaps(TMP_OUT):
    ...
    ebins = np.arange(200)
    mm_e = {
        i: np.zeros((xbins.size - 1, ybins.size - 1, ebins.size - 1), dtype=np.uint)
        for i in range(16)
    }
    for j in range(16):
        for e, x, y in zip(
            np.random.normal(100 + 2 * j, 5, gen_stats),
            np.random.uniform(mm_pitch * j % 4, mm_pitch * (j % 4 + 1), gen_stats),
            np.random.uniform(
                mm_pitch * (3 - j // 4), mm_pitch * (4 - j // 4), gen_stats
            ),
        ):
    ...
    out_base = os.path.join(TMP_OUT, "testplots.ldat")
    eplots = sm_floodmaps(
        min_peak=100, out_base=out_base, ebins=ebins, xbins=xbins, ybins=ybins
    )
    _ = eplots(0, mm_e)
