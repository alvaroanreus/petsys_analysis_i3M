...
def energy_weighted_average(
    channel_pos: Callable, local_indx: int, power: int
) -> Callable:
    """
    ...
    def _average(sm_info):
        ...
        id_charge = np.asarray(
            tuple(map(lambda x: (channel_pos(x[0])[local_indx], x[3]), echans))
        )
...
def calibrate_energies(
    ...
    if time_cal:
        ...
        tcal = (
            pd.read_csv(time_cal, sep=sep)
            .set_index("ID")["MU"]
            .apply(lambda x: 511 / x)
        )
    else:
    ...
    def apply_calibration(event: tuple[list]) -> tuple[list]:
        for sm in event:
            for imp in sm:
                imp[3] *= cal.get(
                    imp[0], 0.0
                )  # Effectively mask channels without calibration
        return event
...
def read_skewfile(skew_file: str) -> pd.Series:
    ...
    skew_vals["id"] = skew_vals.apply(
        lambda r: get_absolute_id(*r[elec_cols]), axis=1
    ).astype("int")
...
def bar_source_dt(bar_xy: np.ndarray, bar_r: float, slab_pos: Callable) -> Callable:
    ...
    def geom_dt(ref_id: int, coinc_id: int) -> float:
        ...
        c = (
            ref_pos[0] * (ref_pos[0] - 2 * bar_xy[0])
            + ref_pos[1] * (ref_pos[1] - 2 * bar_xy[1])
            + c_corr
        )
