...
def test_time_of_flight(TEST_DATA_DIR):
    ...
    *_, slab_mapping = read_ymlmapping(test_yml)
    ...
    assert all(
        np.fromiter(map(flight_time, slab_mapping.values()), float) < max_time * 1e12
    )
...
def test_mm_energy_centroids(TEST_DATA_DIR, DUMMY_EVT):
    ...
    mm_cent = mm_energy_centroids(
        c_calc, chan_map.get_supermodule, chan_map.get_minimodule
    )
